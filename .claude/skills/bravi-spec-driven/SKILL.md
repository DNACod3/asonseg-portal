---
name: bravi-spec-driven
description: Feature planning and implementation with 4 adaptive phases — Specify, Design, Tasks, Execute. Auto-sizes depth by complexity. Creates atomic tasks with verification criteria, atomic git commits, and requirement traceability. Features an independent Verifier (author != verifier, evidence-or-zero), persistent decision log (STATE.md), and test-coverage-matrix-driven tests, plus a self-improving lessons layer that turns verification failures into reusable project-local guidance. Adds four guarantees over plain SDD — must-nots (world-level prohibitions as first-class negative ACs with mandatory negative tests), an Entry Gate that STOPS features blocked on externally-owned decisions, a risk-based sizing floor (must-not/irreversible/foundational features never auto-size below Large), and source-of-truth adapters (when requirements or design already exist upstream — PRD, tickets, design docs, ADRs — the spec indexes them and reuses their IDs instead of re-deriving). Stack-agnostic. Use when (1) Planning features (requirements, design, task breakdown), (2) Implementing with verification and atomic commits, (3) Validating or verifying an implementation against a spec. Triggers on "specify feature", "discuss feature", "design", "tasks", "implement", "validate", "verify work", "UAT", "record decision", "pause work", "resume work". Do NOT use for architecture decomposition analysis (use architecture skills) or technical design docs (use create-technical-design-doc).
license: CC-BY-4.0
metadata:
  author: cfassula
  version: 3.2.0-bravi
  derivedFrom: bravi-spec-driven@3.1.0 (Felipe Rodrigues - github.com/felipfr)
---

# Bravi - Spec-Driven Development

Plan and implement features with precision. Granular tasks. Clear dependencies. Right tools. Zero ceremony.

> **This is the Bravi variant of `bravi-spec-driven` 3.1.0.** It keeps everything from the original (independent Verifier, lessons layer, test coverage matrix) and adds **four guarantees** generalized from the ICE/IDSD method — without importing any ICE artifact dependency. They are woven into the phases below and flagged with 💠:
>
> 1. **Must-nots** — prohibitions on the state of the world captured as first-class negative ACs, each owned by a task and proven by a green negative test ([specify.md](references/specify.md) → [tasks.md](references/tasks.md) → [validate.md](references/validate.md)).
> 2. **Entry Gate** — a feature whose implementation depends on an unresolved, externally-owned decision does NOT enter task breakdown; it gets a structured STOP naming the owner ([tasks.md](references/tasks.md) §0).
> 3. **Risk sizing floor** — auto-sizing never demotes a feature that carries a must-not, touches an irreversible operation, or is foundational (see Auto-Sizing rules).
> 4. **Adapt, don't re-derive** — when requirements/design already exist upstream (PRD, ticket, design doc, ADR), Specify/Design index and reference them with the upstream IDs as canonical; they never fork a second source of truth ([specify.md](references/specify.md), [design.md](references/design.md)).
...
5. 💠 Every **must-not** in the spec must be owned by a task and covered by a **negative test** (asserting the prohibited outcome does not occur) that passes. The Verifier blocks PASS on a missing or red must-not — TestGate green is necessary but not sufficient.
...
**💠 Risk sizing floor (hard rule, not auto-sized):** A feature is classified **Large at minimum** — full spec with requirement IDs and a formal Tasks phase, never Small/Medium — when ANY of these holds:

- It carries at least one **must-not** (a world-level prohibition — see [specify.md](references/specify.md)), OR
- It touches an **irreversible or high-blast-radius operation** (data migration/deletion, payments, auth/authz, external side effects that cannot be rolled back), OR
- It is **foundational** — other planned features depend on its contracts, so getting it wrong propagates.

Rationale: these are exactly the features where per-task gates, negative tests, and the independent Verifier earn their cost. Auto-sizing exists to remove ceremony from cheap mistakes; it must never remove protection from expensive ones. The agent does not get to choose Small/Medium here.
