...
**💠 Upstream design artifacts: adapt, don't re-derive.** If the project already has design-level sources of truth — ADRs, a technical design document, an architecture document — resolve and **reference** them: quote the relevant fragment, cite the anchor (file + section), and design within it. Never re-decide what such an artifact fixed. If the best design for this feature would contradict one of them, that is not a local design decision: surface it to the user as an upstream change request (or a superseding `AD-NNN` when the artifact is project-internal). Same conform-or-supersede discipline as STATE.md decisions, extended to documents.
