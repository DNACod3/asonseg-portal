...
4. 💠 **Must-not verification** — every spec must-not (`[FEAT]-MN-NN`) traced evidence-or-zero to a green negative test; a missing or red must-not forces FAIL (see `validate.md` §6b).
5. **Writes the persisted report** to `.specs/features/[feature]/validation.md` — PASS/FAIL, per-AC evidence (`file:line` + assertion + spec outcome), sensor result (killed/survived per mutation), gate exit results, diff/commit range.
6. **Returns a compact verdict in chat** to the orchestrator.
7. Does **NOT** write, modify, or fix any code or tests — the real working tree is never mutated (sensor mutations run in scratch state only).
...
**Must-nots**: [N/N negative tests green | n/a]
