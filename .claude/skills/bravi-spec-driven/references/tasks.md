...
### 0. 💠 Entry Gate (before any breakdown)

> Two different things gate work in this method — keep them apart. The **Entry Gate** (this step) authorizes the *feature* to enter task breakdown at all. The per-task **gate check** (Gate Check Commands below) is the test-runner level that runs during Execute.

Before breaking the feature into tasks, re-read the spec's **Assumptions & Open Questions**. Look for unresolved items (`Confirmed? = n`) whose **owner is external** — the user/stakeholder, a client, another team, a pending dependency or approval — AND on which the implementation depends.

**If any such item exists → STOP. Do not produce `tasks.md`.** Return a structured stop:

```
⛔ [feature] not entering task breakdown — entry gate open.
   Blocked on: [the unresolved decision / dependency]
   Not mine to resolve: [who owns it]
   Cost of guessing: [what gets built wrong / rework if the owner decides differently]
```

Then offer only what is safe to advance without the blocked input (scaffolding, unaffected slices) — explicitly listing what is NOT safe.

Why this exists: an assumption the agent can default (owner: `agent`) is what the Assumptions table is for. But implementing on top of a decision that belongs to someone else converts their pending choice into your rework — the gate is cheap, the rework is not. When the owner later resolves the item, update the spec's Assumptions table first (`Confirmed? = y`), then re-enter here.
...
**💠 Check 4: Must-Not Ownership** — every `[FEAT]-MN-NN` in the spec is owned by the task(s) that could violate it, and that task's `Tests`/`Done when` includes the **negative test** asserting the prohibited outcome does not occur. A must-not may map to more than one task when several tasks could violate it; a must-not with **no** owning task is a decomposition gap — restructure. Skip this check only when the spec has no Must-Nots section.
