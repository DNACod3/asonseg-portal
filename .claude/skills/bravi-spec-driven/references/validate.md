...
### 6b. 💠 Must-Not Verification (when the spec has must-nots)

Tests passing is necessary but **not sufficient** when the spec declares must-nots. For every `[FEAT]-MN-NN`:

1. Locate its **negative test** — evidence-or-zero: `file:line` + the assertion that the prohibited outcome does not occur. No citation = NOT covered, regardless of how correct the implementation looks.
2. Confirm it is GREEN.
3. A must-not only reaches `Verified` with its negative evidence green — the feature **cannot PASS validation** with a missing or red must-not.
4. Point at least one discrimination-sensor mutation at each must-not's guard (e.g., remove the guard in scratch state, confirm the negative test kills the mutant). A must-not whose negative test survives the removal of its own guard is not protected — fix task.
...
**Must-nots**: [N/N negative tests green | n/a — spec has no must-nots]
...
## Must-Not Verification (if spec has must-nots)

| ID           | SHALL NOT…            | Negative test (`file:line` + assertion) | Green? | Guard mutation killed? |
| ------------ | --------------------- | --------------------------------------- | ------ | ---------------------- |
| [FEAT]-MN-01 | [prohibited outcome]  | `path/to/test.ts:42` — `expect(rows).not.toContainEqual(...)` | ✅ | ✅ |

**Status**: ✅ All must-nots proven / ❌ Missing or red must-not → validation FAIL
...
- 💠 **Must-nots block PASS** — a missing or red negative test on any `[FEAT]-MN-NN` fails the validation, no matter how green everything else is
