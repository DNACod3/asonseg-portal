...
- **Select**: `title, description, requirements, salary, salaryMin, salaryMax, salaryVisible, workRegime, contractType, educationLevelRequired, location, area:{name}, region:{name}, company:{razaoSocial, nomeFantasia}`. Faixa salarial = `salaryMin/Max` (ou `salary` legado) respeitando `salaryVisible`; `companyName` = nome fantasia/razão social (identidade pública — premissa §6). Sem filtro de `status` no `select` (o item já veio `IN_MODERATION` da fila; a action é a fronteira de permissão).
...
| `CV` (kind isolado) sem reader | Sem entrada no dispatcher → `null` → E-006 gracioso | `_moderation_fixture` vazio em prod; conteúdo de candidato (incl. CV) vem por `CANDIDATE_PROFILE`. |
