...
**Current Work:** **Fase 2 — Empresas + Vagas + Moderação — COMPLETA, PASS** (AD-016). Rodada única (1 PR) planejada→implementada→validada em 5 unidades pelo pipeline Planner→Implementer→Verifier, cada USP com Verifier independente. 9 USPs já implementadas foram **refatoradas** (DS AD-014 + consistência, style-only preservando comportamento): U1 Empresas (012/013/014/015), U2 Moderação (016/017), U4 Vagas (020/021/022). 4 USPs **novas** implementadas: U3 (018 inativar conteúdo, 019 sugerir categoria), U5 (023 editar/pausar/arquivar/renovar, 024 expiração automática). ROADMAP: USP-018/019/023/024 marcadas `[x]`; Fase 2 100%. Branch `refactor/fase-2-empresas-vagas-moderacao`. Gates finais no HEAD: typecheck/lint verdes, **995 unit, 325 integração**, build OK, migração `usp024_expiry_reminder` aplica limpa (23/23). U5 teve 1 ciclo fix→re-verify (lacunas na camada de teste rota/UI, fechadas). Ajuste extra em `ci.yml`: passo de seed demo entre integração e Playwright (fixtures E2E d001–d006). Próximo: abrir o PR único; Fase 3 (USP-025) a seguir.
