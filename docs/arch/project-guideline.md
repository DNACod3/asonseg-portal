...
💡 **`migrate deploy` avisa drift; `migrate dev` trava.** O passo 3 acima vale para `prisma migrate deploy` (`npm run db:deploy`/`db:deploy:staging`, e produção) — WARN, não bloqueia. Mas o comando do dia a dia local, `npm run db:migrate` (= `prisma migrate dev`), **não** degrada para warning: ao detectar uma migration já aplicada com conteúdo diferente do checksum registrado, ele **para** e exige resetar o banco de desenvolvimento. Qualquer dev com banco local já provisionado antes de uma migration ser editada in-place precisa rodar `supabase db reset` + `npm run db:deploy` (não os dois sozinhos — `db reset` não aplica as migrations do Prisma) antes do próximo `db:migrate`.
