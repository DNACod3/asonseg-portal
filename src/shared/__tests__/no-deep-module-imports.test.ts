 ...
 *
 * **Exceção documentada (A-07 / nota de conformidade F0A-05):** os 3 pontos em
 * `persons/components/{candidate-form,provider-form}.tsx` são Client Components
 * que precisam importar a Server Action `activateAdditionalRole` (e o tipo
 * `ProfileField`/`PROFILE_FIELD_META`) diretamente do arquivo-fonte, não do
 * barrel `@/modules/identity`: o barrel reexporta código server-only
 * (`./server/session` → `supabase/server.ts` → `next/headers`; e, via
 * `./ports/captchaVerifier` → `container.ts` → `next-cache-invalidation.ts` →
 * `next/cache`), que o Next se recusa a empacotar no bundle do cliente.
 * **Verificado empiricamente**: rotear esses 3 imports pelo barrel quebra
 * `npm run build` (`Failed to compile` — "You're importing a component that
 * needs 'next/headers'/'revalidatePath'... not supported in the pages/
 ...
 * fronteira client/server. Cada um dos 3 imports carrega o comentário de
...
describe('F0-MN-02 — módulos importam-se só pelo barrel (@/modules/<x>)', () => {
  ...
  it('a exceção documentada continua restrita aos 2 arquivos conhecidos (client/server boundary)', () => {
